...
(define (lam [v : (Listof (Pair String PE))] [p : PE]) : Boolean
   (match p
     ...
     [(Cat p1 p2 ann) (and (lam v p1) (lam v p2))]
...
(define (annPE [n : Natural ] [v : (Listof (Pair String PE))] [ p : PE ]) : PE
  (match p
    ...
    [(Cat e d xs) (Cat (annPE n v e) d (cond [(lam v d) (insPE n d xs)]
                                             [else       xs]))]
  )
 )
  ...
  (match p
    ...
    [(Cat e d xs) (let* ([de (der c i v e) ]
                        ...
                        (match de
                           ...
                           [ _  (cat de d
                                    (insPE i (annPE i v d)
                                     (map (lambda ([jp : (Pair Natural PE)]) (cons (car jp) (der c i v (cdr jp))))  xs))
                                )]))
    ]
  )
 )
